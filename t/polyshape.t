...

{
    ...
    cmp_ok( $poly->get_vert(0)->[0] - $verts->[0][0],
        '<', 1e-5, 'get_vert 0 x' );
    cmp_ok( $poly->get_vert(0)->[1] - $verts->[0][1],
        '<', 1e-5, 'get_vert 0 y' );

    cmp_ok( $poly->get_vert(1)->[0] - $verts->[1][0],
        '<', 1e-5, 'get_vert 1 x' );
    cmp_ok( $poly->get_vert(1)->[1] - $verts->[1][1],
        '<', 1e-5, 'get_vert 1 y' );

    cmp_ok( $poly->get_vert(2)->[0] - $verts->[2][0],
        '<', 1e-5, 'get_vert 2 x' );
    cmp_ok( $poly->get_vert(2)->[1] - $verts->[2][1],
        '<', 1e-5, 'get_vert 2 y' );
